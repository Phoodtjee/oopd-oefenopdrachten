...
public class PersonenLijst {
	...
	public ArrayList<Student> getSLBStudenten(Docent SLBer) {
		ArrayList<Student> SLBStudenten = new ArrayList<Student>();
		for(int i = 0; i<this.lijst.size();i++) {
			if(this.lijst.get(i) instanceof Student) {
				Student stud = (Student) this.lijst.get(i);
				if(stud.getSLBer() == SLBer) {
					SLBStudenten.add(stud);
				}
			}
		}
	return(SLBStudenten);
	...
	public String toString() {
		return "PersonenLijst [lijst=" + lijst + "]";
	}
}